...
from ttk_backend.core.utils import camel_to_snake_object_keys, form_data_to_json
...
class FilterFieldsMixin(GenericAPIView):
    ...
    def filter_queryset(self, queryset):
        ...
        for field_name in query_params:
            ...
            if field_name in self.text_filter_fields:
                filters[field_name + '__unaccent__icontains'] = value
                continue
    ...
    def get_query_list_params(self):
        return camel_to_snake_object_keys(dict(self.request.query_params.lists()))

    def get_data_list_params(self):
        return camel_to_snake_object_keys(dict(self.request.data.lists()))

    def get_form_data_params(self):
        data = self.get_data_list_params()
        return form_data_to_json(data)
